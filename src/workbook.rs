        ...
        for defined_name in &self.defined_names {
            ...
            match defined_name.name_type {
                ...
                _ => {
                    ...
                    let sheet_name = defined_name.range.split('!').next().unwrap_or("");
                    if let Some(&index) = sheet_name_to_index.get(sheet_name) {
                        attributes.push(("localSheetId", index.to_string()));
                    } else {
                        // If sheet name is not found, continue to the next defined name
                        continue;
                    }
                }
            }
